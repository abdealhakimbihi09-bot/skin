...
export default function SkinRewards() {
  return (
    <section id="skin-rewards" className="py-24 px-6 relative overflow-hidden bg-dark-bg">
      {/* Background Atmosphere */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 w-[50%] h-[50%] bg-brand-red/5 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-0 right-1/4 w-[50%] h-[50%] bg-brand-blue/5 rounded-full blur-[120px] animate-pulse [animation-delay:2s]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
            className="flex items-center gap-2 text-brand-red font-display font-black uppercase tracking-[0.4em] text-xs mb-4"
          >
            <Sparkles className="w-4 h-4" />
            Limited Series
          </motion.div>
          <h2 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter text-white mb-6">
            SKIN <span className="text-brand-red">REWARDS</span>
          </h2>
          <p className="text-zinc-500 max-w-2xl text-xs uppercase tracking-[0.3em] font-bold">
            Unlock official cinematic skins for the multiverse's greatest legends
          </p>
          <div className="h-1 w-24 bg-gradient-to-r from-transparent via-brand-red to-transparent rounded-full mt-8" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8 justify-items-center">
          {SKIN_REWARDS.map((item, idx) => (
            <motion.div
              key={item.id}
              ...
              className="group relative aspect-[3/4.5] w-full max-w-[320px] sm:max-w-none flex flex-col bg-[#05080f] rounded-[2rem] p-[2px] overflow-hidden transition-all duration-700 hover:shadow-[0_0_60px_rgba(255,255,255,0.05)]"
              style={{
                background: `linear-gradient(135deg, ${item.color}80, transparent, ${item.color}40)`
              }}
            >
              ...
              <div className="relative h-full w-full bg-[#0a0a0a]/95 backdrop-blur-3xl rounded-[1.9rem] flex flex-col overflow-hidden border-shine">
                ...
                <div className="absolute top-4 left-4 z-20 flex items-center gap-2 px-3 py-1.5 bg-black/60 border border-white/20 rounded-sm backdrop-blur-md shadow-2xl">
                  ...
                  />
                  <span className="text-[9px] font-black uppercase text-white tracking-[0.2em]">Exclusive Skin</span>
                </div>
                ...
                <div className="relative flex-[1.2] overflow-hidden flex items-center justify-center p-4">
                  ...
                  <div 
                    className="absolute inset-0 opacity-40 blur-[100px] animate-pulse"
                  ...
                  <motion.img 
                    ...
                    src={item.image} 
                    alt={item.name}
                    className="h-full w-full object-contain z-10 transition-transform duration-1000 drop-shadow-[0_0_40px_rgba(0,0,0,0.8)]"
                ...
                <div className="relative p-8 pt-0 bg-gradient-to-b from-transparent to-[#0a0a0a] z-20">
                  <div className="mb-6 relative">
                    <p className="text-[10px] text-white/40 font-black uppercase tracking-[0.4em] mb-2 pl-1 border-l border-brand-red/30">
                      {item.name}
                    </p>
                    <h3 
                  ...
                  >
                    <span className="relative z-10 flex items-center justify-center gap-3">
                       Unlock Reward <Zap className="w-4 h-4 fill-current" />
                    </span>
                ...
                <div className="absolute bottom-0 right-0 w-8 h-8 border-r-2 border-b-2 border-white/10 rounded-br-[1.9rem] mix-blend-overlay" />
                <div className="absolute top-0 left-0 w-8 h-8 border-l-2 border-t-2 border-white/10 rounded-tl-[1.9rem] mix-blend-overlay" />
              </div>
            </motion.div>
          ))}
        </div>
